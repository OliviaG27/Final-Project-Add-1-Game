    ...
    
{
    ...
    func updateTimeLabel() {
     ...
     @IBAction func inputFieldDidChange()
       {
        guard let numberText = numberLabel?.text, let inputText = imputField?.text else {
       ...
       guard inputText.count == 4 else {
        ...
        func finishGame()
        {
            ...
            let alert = UIAlertController(title: "Time's Up!", message: "Your time is up! You got a score of \(score) points. Awesome!", preferredStyle: .alert)
                alert.addAction(UIAlertAction(title: "Okay, start new game", style: .default, handler: nil))
